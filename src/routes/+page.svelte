...
<script>
  ...
  onMount(async () => {
    ...
    if (threeLoad.status !== 'fulfilled' || pointerLoad.status !== 'fulfilled') {
      fallbackMode = true;
      message = '3D renderer could not load; fallback world map is shown below.';
      return () => {};
    }
...
<main>
  <div class="overlay top">
    ...
    <p class="subtitle">Classic Roblox-like controls: WASD/Arrows move, Space jump, Shift sprint, mouse look.</p>
  </div>
